...
dfseg <-dfseg %>% mutate(xaxisTime = c(1,2,1,2))


ggplot(df,aes(y=Y,x=xaxisTime,color=as.factor(Control)))+geom_point()+
  guides(color=guide_legend(title="Group"))+
  geom_vline(aes(xintercept=1.5),linetype='dashed')+
  scale_color_colorblind()+
  scale_x_continuous(
  ...
  geom_line(data = dfseg, mapping = aes(y=mean_Y,x=xaxisTime, color = as.factor(Control)), size=2) +
  geom_segment(aes(x=1,xend=2,y=3.903093,yend=4.867091,  linetype = "counterfactual"), color="red", size=1.5) +
  geom_segment(aes(x=2,xend=2,y=4.867091,yend=6.618977, linetype="DID"), color = "blue",size=1.5) +
  scale_linetype_manual("Estimates", values=c("DID" = 1, "counterfactual" = 2), labels=c("counterfactual", "DID"))
