...
\usepackage{graphicx}
\usepackage[backend=bibtex, style=authoryear, maxcitenames=3]{biblatex}
\usepackage{makecell}
\usepackage{textcomp}
\usepackage{listings}
...
\section{Synthetic Controls}
	...
	\subsection{estimation}
		...
		\begin{frame}{estimation}
			...
			\end{equation}
			\\ \vspace*{.25cm}
			This matching is often performed on lagged outcomes $Y_{t-(1,...,T)}$ and other covariates. \onslide<2->{So, in simpler terms, $	\| X_{treat} - X_{control} W \|$  which resembles a balancing approach (á la matching).}
			\\ \vspace*{.25cm}
	...
	\subsection{current development}
		...
		\begin{frame}{intermediate summary}
			...
			\begin{itemize}
				...
				\item so far, has not been \text{widely} applied (for examples see \cite{Abadie2020b})
