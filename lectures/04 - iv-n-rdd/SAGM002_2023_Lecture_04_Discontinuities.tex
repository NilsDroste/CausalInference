...
\section{Introduction}
	\begin{frame}{Introduction}
		\textbf{\textit{Synopsis}}: Today, we will be looking into situations where exogenous variations can be exploited for a quasi-random treatment assignment \\ \vspace*{.25cm}
		In particular, we will develop an understanding of\\ \vspace*{.25cm}
		\begin{itemize}
			\item<2-> instrumental variables approaches
			\item<3> regression discontinuity designs
		\end{itemize}
	\end{frame}
	...
	\subsection{formalities}
		...
		\begin{frame}{notation}
			...
			\begin{itemize}
				\item []
				\begin{itemize}
					\item<3-> here the LATT (!) is given by $P(z) \cdot E \lbrack Y_i(1) - Y_i(0)|D_i(z) = 1 \rbrack $
					\item<4-> as long as participation $P(z)>P(0)$ and $D_i(z) \geq D_i(0) \forall i$, aka monotonic \\ (or $\leq$, respectively)
				\end{itemize}
			\end{itemize}
			\vspace*{.25cm}
		...
		\begin{frame}{Who's gonna be "treated"}
			...
			\begin{itemize}
				...
				\item<4-> allows valid estimate of LATE / LATT, but may not always be realistic
			\end{itemize}
		\end{frame}
