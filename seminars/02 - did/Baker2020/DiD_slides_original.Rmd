...
- Here firms are randomly assigned to one of 50 states. The 50 states are randomly assigned into one of 5 treatment groups $G_g$ based on treatment being initiated in 1985, 1991, 1997, 23003, and 2009. 
...
bind_rows(
  ...
  # keep just the variables we need
  dplyr::select(df, estimate, conf.low, conf.high) %>% 
...
bind_rows(data %>% mutate(df = "1999-2017"), data_2010 %>% mutate(df = "1999-2010")) %>% 
  ...
  group_by(df) %>% 
  mutate(t = c(-3:-2, 0:3)) %>% 
  dplyr::select(df, t, estimate, conf.low, conf.high) %>% 
  # add in data for year -1
  bind_rows(tibble(
