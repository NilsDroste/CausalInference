###########################################################################
# Application: Aggregated results - Baseline specification
###########################################################################
...
e_select <- (ES_aggte$egt >= 0) & ((ES_aggte$egt <= maxe))
...
att_never_time <- mean(ES_aggte$att.egt[e_select])
att_never_time_se <- bootst_es$se

# Now do this for the "simple estimator"
aggte_simple <- did::aggte(out.never.violrate, type = "simple")
inf_func_simple <- aggte_simple$inf.function$simple.att
bootst_simple <- did::mboot(inf_func_simple, par_boot)
# Save point estimates and standard errors
att_never_simple <- aggte_simple$overall.att
...
bootst_es <- did::mboot(inf_func, par_boot)
# Save point estimates and standard errors
att_ny_time <- mean(ES_aggte$att.egt[e_select])
att_ny_time_se <- bootst_es$se

# Now do this for the "simple estimator"
aggte_simple <- did::aggte(out.ny.violrate, type = "simple")
inf_func_simple <- aggte_simple$inf.function$simple.att
bootst_simple <- did::mboot(inf_func_simple, par_boot)
# Save point estimates and standard errors
att_ny_simple <- aggte_simple$overall.att
...
bootst_es <- did::mboot(inf_func, par_boot)
# Save point estimates and standard errors
att_nyall_time <- mean(ES_aggte$att.egt[e_select])
att_nyall_time_se <- bootst_es$se

# Now do this for the "simple estimator"
aggte_simple <- did::aggte(out.nyall.violrate, type = "simple")
inf_func_simple <- aggte_simple$inf.function$simple.att
bootst_simple <- did::mboot(inf_func_simple, par_boot)
# Save point estimates and standard errors
att_nyall_simple <- aggte_simple$overall.att
...
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
twfe.violrate = lfe::felm(violrate ~ after | (state + year)| 0 | state,
                          data = grooms, 
                          exactDOF = T,
...
summary.never.grooms <- rbind(
  ...
  twfe.violrate = twfe.violrate$beta,
  twfe.se.violrate = twfe.violrate$cse
)
...
xlsx::write.xlsx(summary.grooms, here("tables/grooms-baseline.xlsx"))
