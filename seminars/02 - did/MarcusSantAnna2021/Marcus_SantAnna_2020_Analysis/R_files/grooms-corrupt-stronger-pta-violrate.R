...
# Rule out corruption-specific trends
###########################################################################
...
plot.event.never.violrate <- ggplot2::ggplot(event.study, aes(x=year, y=att, ymin = (att-cv*att.se),
  ...
  scale_y_continuous(limits =c(-0.20,0.06), breaks = round(seq(-0.20,0.04,0.04),2)) +
  scale_x_discrete(breaks=dabreaks, labels=as.character(dabreaks))+
  theme(plot.title = element_text(color="black", face="bold", size = 12))+
  ggtitle("(b) Event study using the never-treated units as comparison group, not allowing for corruption-specific trends")
...
plot.event.ny.violrate <- ggplot2::ggplot(event.study, aes(x=year, y=att, ymin = (att-cv*att.se),
  ...
  scale_x_discrete(breaks=dabreaks, labels=as.character(dabreaks))+
  theme(plot.title = element_text(color="black", face="bold", size = 12))+
  ggtitle("(c) Event study using the not-yet-treated units as comparison group, not allowing for corruption-specific trends")
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
...
plot.event.nyall.violrate <- ggplot2::ggplot(event.study, aes(x=year, y=att, ymin = (att-cv*att.se),
  ...
  scale_x_discrete(breaks=dabreaks, labels=as.character(dabreaks))+
  theme(plot.title = element_text(color="black", face="bold", size = 12))+
  ggtitle("(d) Event study using all not-yet-treated units as comparison group, not allowing for corruption-specific trends")
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
...
violrate.plots
ggsave(here("plots/event-study-violrate-corrupt2.pdf"), width = 12, height = 14, units = "in",
       plot = violrate.plots)
dev.off()
